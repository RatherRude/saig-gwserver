...
function requestGeneric($request, $preprompt = '', $queue = 'AASPGQuestDialogue2Topic1B1Topic',$lastDataAmount=10,$tag='')
{
    ...
    $head[] = ['role' => 'user', 'content' => '('.$PROMPT_HEAD.$GLOBALS['HERIKA_PERS']];
    $prompt[] = ['role' => 'assistant', 'content' => $request];
    $foot[] = ['role' => 'user', 'content' => $GLOBALS['PLAYER_NAME'].':' . $preprompt];
    ...
    $GLOBALS['DEBUG_DATA']['OPENAI_LAG']=(microtime(true)-$startTime);
